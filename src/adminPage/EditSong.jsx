...
import '../uploadPage/uploadCss.css';
...
export default function EditSongs(props) {
  ...
  return (
    <div id= "edit-view" className = "below-head-view-container">
    <EditForm />
    </div>
  )
}
...
export function EditForm(props) {
  ...
  let searchParam = document.getElementById("edit-song-search").value
  trackPromise(
    axios({
      method: 'post',
      url: BASEURL + '/edit-lyrics/'+searchParam,
      data: formData,
  ...
  function handleEditSongSearch() {
    let param = document.getElementById("edit-song-search").value
    if(param.length > 8){
...
const handleTextArea = (e)=>{
  ...
  <input type = "search" name = "songId" id = "edit-song-search"
  placeholder = "enter id of the song to edit" />
  <button id = "edit-search-button" onClick = {handleEditSongSearch}>search</button>
  </div>
  ...
  <input type = "text" id= "edit-release-date" name="releaseDate" value = {releaseDate}
  className = "upload-input-field" onChange={handleInputChange} required/>
  </div>
  <div className = "input-container">
